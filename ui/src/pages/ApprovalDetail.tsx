...
export function ApprovalDetail() {
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "승인에 실패했습니다"),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "거부에 실패했습니다"),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "수정 요청에 실패했습니다"),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "재제출에 실패했습니다"),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "댓글 작성에 실패했습니다"),
  });
  ...
  const deleteAgentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : "삭제에 실패했습니다"),
  });
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">승인이 확인되었습니다</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  요청한 Agent에게 이 승인 건과 연결된 Issue를 검토하도록 알림이 전송되었습니다.
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">연결된 Issue</p>
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              연결된 Issue는 요청한 Agent가 후속 조치를 취하고 종료할 때까지 열린 상태로 유지됩니다.
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              <Link to="/costs" className="underline underline-offset-2">/costs</Link>의 Budget 관리에서 이 Budget 정지를 해결하세요.
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm("거부된 Agent를 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.")) return;
